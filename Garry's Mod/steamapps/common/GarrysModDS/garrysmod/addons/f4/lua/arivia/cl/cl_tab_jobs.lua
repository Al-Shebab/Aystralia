...
function PANEL:Init( )
    ...
    if Property.Value.name == team.GetName(mod.pnl.root.CurJob) then
    ...
    table.sort  ( cats, function( a, b ) return ( a.sortOrder or 999 ) < ( b.sortOrder or 999 ) end )
    ...
    */
    ...
    for _, cat in pairs( cats ) do
        ...
        */
        ...
        local jobs_bHasErr = 0
        for k, v in pairs( cat.members ) do
            if v.name == team.GetName( mod.pnl.root.CurJob ) then jobs_bHasErr = jobs_bHasErr + 1 end
            if not arivia.tab.jobs.bShowUnavail and not mod:bCanJob( v ) then jobs_bHasErr = jobs_bHasErr + 1 end
        end
        ...
        */
        ...
        if jobs_bHasErr >= count then continue end

        /*
        *	cats > create
        */

        local category      = cat.name and self:CreateCat( cat.name ) or self:CreateCat( lng.CategoryOther )
        ...
        */
        ...
        for _, v in pairs( cat.members ) do
            ...
            if v.name == team.GetName(mod.pnl.root.CurJob) then continue end
            if not arivia.tab.jobs.bShowUnavail and not mod:bCanJob( v ) then continue end
            ...
            function ListItem:Paint( w, h )
                ...
                local clr_btn_n = arivia.tab.jobs.clr_btn_n
                local txtColor = textNormal

                if ListItem:IsHovered( ) or ListItem:IsDown( ) then
                    clr_btn_n = arivia.tab.jobs.clr_btn_h
                    txtColor = textHover
                end
                ...
                if arivia.tab.jobs.bXpEnabled and LevelSystemConfiguration then
                ...
                if LevelSystemPrestigeConfiguration and arivia.tab.jobs.bPrestigeEnabled then
                ...
                if (arivia.tab.jobs.bXpEnabled and ( LevelSystemConfiguration ) ) or (arivia.tab.jobs.bPrestigeEnabled and ( LevelSystemPrestigeConfiguration or v.prestige ) )  then
                    originNameX = 11
                end

                draw.DrawText( objectName, 'AriviaFontObjectListName', 65, originNameX, arivia.tab.jobs.clr_txt or Color( 255, 240, 244 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )

                if arivia.tab.jobs.bXpEnabled then
                    if LevelSystemConfiguration then
                        ...
                        if arivia.tab.jobs.bPrestigeEnabled and LevelSystemPrestigeConfiguration then
                            draw.DrawText( prestigeText, 'AriviaFontObjectLevel', 140, 35, Color( 255, 240, 244, 100 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
                        end
                    end
                elseif arivia.tab.jobs.bPrestigeEnabled then
                ...
                draw.DrawText( GAMEMODE.Config.currency .. v.salary, 'AriviaFontObjectPrice', w - 31, cfg.ItemAmountH or 20, Color( 255, 255, 255, 120 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                ...
                -----------------------------------------------------------------
                if arivia.tab.jobs.bFadeUnavail and jobUnavailable then
                    draw.RoundedBox( 0, 0, 0, w, h, arivia.tab.jobs.clr_unavail or Color( 20, 20, 20, 200 ) )
                end
            ...
            PlayerModel.Paint = function (self, w, h)
                ...
                if jobPlayerMax == 0 then
                    jobPlayerMax = lng.JobMaxUnlimited
                end
    ...
    Property.Item.ModelObject.Paint = function( s, w, h )
        oldPaint( s, w, h )
    ...
    Property.Item.SkinPrev.Paint = function( self, w, h )
        ...
        draw.SimpleText( lng.SkinPrevious, 'AriviaFontCloseGUI', w / 2, h / 2, Color(230, 230, 230, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    Property.Item.SkinNext.Paint = function( self, w, h )
        ...
        draw.SimpleText( lng.SkinNext, 'AriviaFontCloseGUI', w / 2, h / 2, Color(230, 230, 230, 255), 1, 1)
    end
    ...
    Property.Item.ButtonAction.PerformLayout = function( )
        if Property.Value.vote then
            Property.Item.ButtonAction.Text = string.upper( lng.JobCreateVote )
            ...
            Property.Item.ButtonAction.Text = string.upper( lng.JobStartNewJob )
            Property.Item.ButtonAction.DoClick = function( )
                DarkRP.setPreferredJobModel( Property.Value.team, Property.Item.ModelObject:GetModel( ) )
        ...
        if !mod:bCanJob(Property.Value, true) then
    ...
    Property.Item.ButtonAction.Paint = function( self, w, h )
        ...
        if !mod:bCanJob(Property.Value) then
            clr_btn_n = arivia.tab.jobs.clr_unavail
            jobStatus = ( !( LocalPlayer( ):IsAdmin( ) or LocalPlayer( ):IsSuperAdmin( ) ) and Property.Value.admin == 1 ) and lng.JobUnavailable .. ' - ' .. lng.JobAdminOnly or lng.JobUnavailable
        end
        ...
        if arivia.tab.jobs.bXpEnabled then
            ...
            if itemLevel ~= nil and itemLevel then
                if itemLevel >  PlayerLevel then
                    clr_btn_n = arivia.tab.jobs.clr_unavail
                    jobStatus = string.upper(lng.InsufficientLevel)
                end
            end
        end
        ...
        if LevelSystemConfiguration and Property.Value.prestige then
            ...
            if Property.Value.prestige ~= nil then
                if Property.Value.prestige >  PlayerPrestigeAmount then
                    clr_btn_n = arivia.tab.jobs.clr_unavail
                    jobStatus = string.upper(lng.NotEnoughPrestige)
                end
            end
        end

        draw.RoundedBox( 0, 0, 0, w, h, clr_btn_n )
        draw.SimpleText( jobStatus, 'arivia_sel_btn_select', w / 2, h / 2, Color(230, 230, 230, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
    end
...
vgui.Register( 'arivia_tab_jobs', PANEL, 'DPanel' )