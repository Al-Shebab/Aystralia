...
function PANEL:Init( )
    ...
    if FoodItems[1] == nil then
        Property:Remove( ) return
    end

    Property.Value = FoodItems[1]
    ...
    for i = 0, 0 do
        for k, v in ipairs(FoodItems) do
            ...
            if !mod:bCanBuyFood(v) then continue end
            ...
            function ListItem:Paint( w, h )
                ...
                local clr_btn_n = arivia.tab.food.clr_btn_n
                local txtColor = textNormal
                if ListItem:IsHovered( ) or ListItem:IsDown( ) then
                    clr_btn_n = arivia.tab.food.clr_btn_h
                    txtColor = textHover
                end
                ...
                if arivia.tab.food.bXpEnabled and LevelSystemConfiguration then
                ...
                if LevelSystemPrestigeConfiguration and arivia.tab.food.bPrestigeEnabled then
                ...
                if (arivia.tab.food.bXpEnabled and ( LevelSystemConfiguration ) ) or (arivia.tab.food.bPrestigeEnabled and ( LevelSystemPrestigeConfiguration or v.prestige ) )  then
                    originNameX = 11
                end
                ...
                draw.DrawText( objectName, 'AriviaFontObjectListName', 65, originNameX, arivia.tab.food.clr_txt or Color( 255, 240, 244 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )

                if arivia.tab.food.bXpEnabled then
                    if LevelSystemConfiguration then
                        ...
                        if arivia.tab.food.bPrestigeEnabled and LevelSystemPrestigeConfiguration then
                            draw.DrawText( prestigeText, 'AriviaFontObjectLevel', 140, 35, Color( 255, 240, 244, 100 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
                        end
                    end
                elseif arivia.tab.food.bPrestigeEnabled then
    ...
    Property.Item.ModelObject:SetModel(FoodItems[1].model)
    ...
    if cfg.desc.enabled then
        ...
        Property.Item.Description.PerformLayout = function( )
            local foodEntity    = Property.Value.name
            local foodEnergy    = Property.Value.energy or 0
            local text          = 'Energy: ' .. foodEnergy .. '\n\n\n'

            Property.Item.Description:SetText( cfg.desc.list[foodEntity] and cfg.desc.list[foodEntity] .. '\n\nEnergy: ' .. foodEnergy .. '\n\n\n' or text )
        end
    ...
    Property.Item.ButtonAction.PerformLayout = function( )
        if mod:bCanBuyFood(Property.Value) then
            if (LocalPlayer( ).DarkRPVars.money or 0) < Property.Value.price then
                ...
                Property.Item.ButtonAction.DoClick = function( )
                    RunConsoleCommand('darkrp', 'buyfood', Property.Value.name)
                end
            end
        else
...
vgui.Register( 'arivia_tab_food', PANEL, 'DPanel' )