...
function ENT:Draw()
	...
	cam.Start3D2D( pos, ang, 0.11 )
		...
		elseif LocalPlayer().BankRobberyCountdown and LocalPlayer().BankRobberyCountdown > CurTime() then
			...
			if table.HasValue( CH_BankVault.Config.AllowedTeams, team.GetName( LocalPlayer():Team() ) ) then
			...
			if CH_BankVault.Config.UseRequiredModels then
				if table.HasValue( CH_BankVault.Config.RequiredModels, LocalPlayer():GetModel() ) then
