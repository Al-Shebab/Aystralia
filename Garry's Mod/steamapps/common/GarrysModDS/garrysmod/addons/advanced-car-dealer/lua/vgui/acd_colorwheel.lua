...
AccessorFunc( PANEL, 'm_pickerPos', 'PickerPos' ) -- Need timer !
AccessorFunc( PANEL, 'm_color', 'Color' )
AccessorFunc( PANEL, 'm_darkness', 'Darkness', FORCE_NUMBER )
AccessorFunc( PANEL, 'm_alpha', 'Alpha', FORCE_NUMBER )
AccessorFunc( PANEL, 'm_editing', 'Editing', FORCE_BOOL )
AccessorFunc( PANEL, 'm_handleRadius', 'HandleRadius', FORCE_NUMBER )
AccessorFunc( PANEL, 'm_borderSize', 'BorderSize', FORCE_NUMBER )
...
function PANEL:Init( )
    ...
    function self:SetSize( w, h )
        ...
        self.oldSetSize( self, w, w )
    end

    self:SetBorderSize( 3 )
    self:SetBorderColor( Color( 200, 200, 200, 255 ) )
...
local colorwheel_mat = Material( "materials/cardealer/colorwheel.png", "smooth noclamp alphatest" )

function PANEL:Paint( w, h )
    draw.RoundedBox( w, 0, 0, w, h, self.m_borderColor )

    surface.SetDrawColor( 255, 255, 255, 255 )
    surface.SetMaterial( colorwheel_mat )
    ...
    local handleColor = Color( 255, 255, 255, 255 )
...
function PANEL:Think( )
    if ( self:GetEditing( ) && self:IsHovered( ) ) then
        ...
        ang = ang + math.pi
        ang = -ang
        -- print( ang, math.deg( ang ) )
        -- print( math.deg( ang ) > 0 and math.deg( ang ) or math.abs( math.deg( ang ) - 180 ))
...
derma.DefineControl( 'CarDealer.ColorWheel', '', PANEL, 'DPanel' )
