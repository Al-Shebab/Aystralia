...
function PANEL:Init()
    ...
    self.choises.ResizeChilds = function()
        ...
        for k,v in pairs(childs) do
            v:SetWide(self.choises:GetWide() / count)
        end
    end
...
function PANEL:addInput(type, placeholder)
    ...
    element:Dock(BOTTOM)
    element:DockMargin(margin, margin, margin, margin)
    ...
    self:SetTall(self:GetTall() + element:GetTall() + (margin * 2))
...
function PANEL:addChoise(title, func)
    ...
    choise:Dock(LEFT)
    choise:DockMargin(margin * .5,0,margin * .5,0)
