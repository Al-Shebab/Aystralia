...
if( SERVER ) then
	hook.Add( "playerCanChangeTeam", "NoChangeTeamWhenRAPE", function( _p, _, _ )
		if( _p.Raping ) then
			DarkRP.notify( _p, 0, 7, "You can't change team while being raped." );
			return false;
		end
	end );
end
...
concommand.Add( SWEP.PRIMARYPW, function( ply, cmd, args )
	...
	plyVictim.SavedWeapons = {}
	for k,v in pairs(plyVictim:GetWeapons()) do
		plyVictim.SavedWeapons[v:GetClass()] = true
	end
	...
	timer.Simple( RapeLength, function()
		...
		if plyAttacker and plyAttacker:IsValid() then
			...
			timer.Simple( 0.1,
			function()
			if not ply.SavedWeapons then return end
			for k,v in pairs( ply.SavedWeapons ) do
				ply:Give( k )
			end
			end )
		...
		if plyVictim and plyVictim:IsValid() then
			if VictimType == 1 then
				...
				for k, v in pairs( plyVictim.SavedWeapons ) do
					plyVictim:Give( k )
				end
