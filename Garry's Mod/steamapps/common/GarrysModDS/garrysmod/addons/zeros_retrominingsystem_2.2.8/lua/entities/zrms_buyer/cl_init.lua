...
function ENT:DrawDetailInfo()
	...
	cam.Start3D2D(Pos, Ang, 0.1)
		...
		local aIron = ": " .. zrmine.config.Currency .. math.Round(zrmine.config.BarValue["Iron"] * (self:GetBuyRate() / 100))
		local aBronze = ": " .. zrmine.config.Currency .. math.Round(zrmine.config.BarValue["Bronze"] * (self:GetBuyRate() / 100))
		local aSilver = ": " .. zrmine.config.Currency .. math.Round(zrmine.config.BarValue["Silver"] * (self:GetBuyRate() / 100))
		local aGold = ": " .. zrmine.config.Currency .. math.Round(zrmine.config.BarValue["Gold"] * (self:GetBuyRate() / 100))
