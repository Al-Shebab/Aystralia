...
function ENT:DrawInfo()
	...
	if (amount > 0) then
		cam.Start3D2D(Pos, Ang, 0.1)
			...
			if zrmine.default_colors[rtype] then
				surface.SetDrawColor(zrmine.default_colors[rtype])
			end
			surface.SetMaterial(zrmine.default_materials["Circle"])
			surface.DrawTexturedRect(-cSize / 2, -cSize / 2, cSize, cSize)
			draw.NoTexture()

			draw.DrawText(rtype, "zrmine_resource_font1", 0, -50, zrmine.default_colors["white02"], TEXT_ALIGN_CENTER)
