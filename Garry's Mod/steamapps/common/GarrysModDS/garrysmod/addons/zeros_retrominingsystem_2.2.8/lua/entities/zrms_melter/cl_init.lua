...
function ENT:Draw_CoalInfo()
	local nPos = self:GetPos() + self:GetForward() * 80 + self:GetUp() * 10
	local nAng = self:GetAngles()
	nAng:RotateAroundAxis(self:GetRight(), 90)
	nAng:RotateAroundAxis(self:GetForward(), -90)
	...
	cam.Start3D2D(Pos, Ang, 0.1)
		...
		draw.DrawText(amount .. zrmine.config.BuyerNPC_Mass, "zrmine_screen_font2", 0, 10, zrmine.default_colors["white02"], TEXT_ALIGN_CENTER)
...
function ENT:Draw_MainInfo()
	...
	if attach then
		...
		local paintColor = zrmine.default_colors["red03"]
		local cState = self:GetCurrentState()

		if (cState == "WAITING_FOR_ORDER") then
			status = zrmine.language.Melter_InsertOre
		elseif (cState == "CHAMBER_FULL" and self:GetCoalAmount() < zrmine.config.Melter_Vars[resourceType].CoalAmount) then
			status = zrmine.language.Melter_NeedCoal
		elseif (cState == "MELTING") then
			status = zrmine.language.Melter_Melting
		end

		if (resourceType == "Iron") then
			paintColor = zrmine.default_colors["Iron"]
		elseif (resourceType == "Bronze") then
			paintColor = zrmine.default_colors["Bronze"]
		elseif (resourceType == "Silver") then
			paintColor = zrmine.default_colors["Silver"]
		elseif (resourceType == "Gold") then
			paintColor = zrmine.default_colors["Gold"]
		end
		...
		local nPos = attach.Pos + self:GetRight() * 0.2
		local nAng = self:GetAngles()
		nAng:RotateAroundAxis(self:GetForward(), -90)
		cam.Start3D2D(nPos, nAng, 0.1)
			draw.RoundedBox(0, -105, -65, 210, 130, zrmine.default_colors["grey02"])
		...
		if (amount > 0) then
			...
			rtype = zrmine.language.Ore_Empty
		end

		local Pos = self:GetAttachment(self:LookupAttachment("screen")).Pos + self:GetRight() * 0.2 + self:GetForward() * 1
		local Ang = self:GetAngles()
		Ang:RotateAroundAxis(self:GetForward(), 90)
		cam.Start3D2D(Pos, Ang, 0.1)

			if (status ~= nil) then
				draw.DrawText(rtype, "zrmine_screen_font2", 0, -50, zrmine.default_colors["white02"], TEXT_ALIGN_CENTER)
				draw.DrawText(status, "zrmine_screen_font3", 0, 16, zrmine.default_colors["yellow01"], TEXT_ALIGN_CENTER)
				draw.DrawText(amount .. zrmine.config.BuyerNPC_Mass, "zrmine_screen_font1", 0, -20, zrmine.default_colors["white02"], TEXT_ALIGN_CENTER)
				surface.SetDrawColor(zrmine.default_colors["white02"])
				surface.SetMaterial(zrmine.default_materials["Scale"])
				surface.DrawTexturedRect(-125, -53, 75, 105)
				draw.NoTexture()
			end
		...
		if IsValid(self.ClientModels["Filling"]) then
			...
			if self.LastResourceAmount ~= rAmount then
				...
				if rAmount <= 0 then
					...
					//self.ClientModels["Filling"]:SetPos(self:LocalToWorld(Vector(0,0,self.ResrcFill_Height)))
