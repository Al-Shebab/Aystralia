do
	...
	local module_filename = "gmsv_chttp_" .. ((is_windows and (arch == "x64" and "win64" or "win32")) or (is_linux and (arch == "x64" and "linux64" or "linux"))) .. ".dll"

	if ((is_windows or is_linux) and not CHTTP) then
		if (file.Exists("bin/" .. module_filename, "LUA")) then
			...
			if (not succ) then
				...
				GAS:print("[bLogs] You may want to report this here: https://github.com/timschumi/gmod-chttp/issues", GAS_PRINT_COLOR_BAD, GAS_PRINT_TYPE_WARN)
				MsgC("\n")
			end
		else
			MsgC("\n")
			GAS:print("[bLogs] Could not find garrysmod/lua/bin/" .. module_filename .. " on your server! Discord webhooks cannot be dispatched.", GAS_PRINT_COLOR_BAD, GAS_PRINT_TYPE_WARN)
...
function GAS.Logging:SuperiorHook(hook_name, identifier, func)
	return GAS:SuperiorHook(hook_name, "logging:" .. identifier, func)
end
function GAS.Logging:InferiorHook(hook_name, identifier, func)
	return GAS:InferiorHook(hook_name, "logging:" .. identifier, func)
end
...
function GAS.Logging:GetEntityCreator(ent)
	...
	elseif (IsValid(ent.FPPOwner)) then
		return ent.FPPOwner
	elseif (ent.IsDoor and ent:IsDoor() == true and IsValid(door:getDoorOwner())) then
