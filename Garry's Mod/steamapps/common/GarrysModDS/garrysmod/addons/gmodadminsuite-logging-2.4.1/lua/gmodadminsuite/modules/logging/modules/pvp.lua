...
PVP_COMBAT_MODULE:Setup(function()
	...
	function GAS.Logging.PvP.PVP_EVENT:Init(instigator, victim, dmginfo)
		...
		GAS.Logging.PvP.OngoingEvents[instigator_id .. victim_id] = self
		...
		self.Properties[GAS.Logging.PvP_FLAGS] = { -- flags
			...
			[GAS.Logging.PvP_FLAG_LAWENFORCEMENT] = (DarkRP ~= nil and (instigator:isCP() or victim:isCP())) or nil,
			[GAS.Logging.PvP_FLAG_ONGOING] = true,

		}

		local combat_scene = table.Copy(GAS.Logging.PvP.CombatScene)
	...
	function GAS.Logging.PvP.PVP_EVENT:End()
		...
		GAS.Logging.PvP.OngoingEvents[self.Properties[GAS.Logging.PvP_INSTIGATOR] .. self.Properties[GAS.Logging.PvP_VICTIM]] = nil -- remove event from ongoing events table
	...
	function GAS.Logging.PvP:GetEvent(attacker, defender, dmginfo)
		if (GAS.Logging.PvP.OngoingEvents[attacker:AccountID() .. defender:AccountID()]) then
			return GAS.Logging.PvP.OngoingEvents[attacker:AccountID() .. defender:AccountID()]
		elseif (GAS.Logging.PvP.OngoingEvents[defender:AccountID() .. attacker:AccountID()]) then
			return GAS.Logging.PvP.OngoingEvents[defender:AccountID() .. attacker:AccountID()]
		else
			local new_event = table.Copy(GAS.Logging.PvP.PVP_EVENT)
	...
	local function EntityTakeDamage(victim, dmginfo, took)
		if (not took or not victim:IsPlayer() or victim:IsBot() or victim:HasGodMode() or not victim:Alive() or dmginfo:GetDamage() <= 0) then return end
		...
		elseif ((IsValid(attacker) and attacker:IsVehicle()) or (IsValid(inflictor) and inflictor:IsVehicle())) then
			...
			if (IsValid(driver)) then
				if (driver == victim) then
					if (GAS.Logging.PvP.PlayerEvents[account_id]) then
						for _,event in ipairs(GAS.Logging.PvP.PlayerEvents[account_id]) do
							pvp_event:AddDamage("pvp_vehicle_damage_self", victim, nil, vehicle, dmginfo, GAS.Logging:FormatPlayer(victim), GAS.Logging:FormatVehicle(vehicle, true))
						end
					end
				else
	...
	GAS.Hooking:InferiorHook("DoPlayerDeath", "logging:PvP:TrackDeathDmg", function(victim, attacker, dmginfo)
