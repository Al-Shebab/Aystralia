...
function ENT:Finish_VFX_SFX()
    zfs.f.CreateEffectTable(nil,  "zfs_sfx_melon_finish", self, self:GetAngles(), self:GetPos(),nil)
    return self.BaseClass.Finish_VFX_SFX(self)
end

function ENT:Interact_VFX_SFX()
    zfs.f.CreateEffectTable("zfs_melon",  "zfs_sfx_melon", self, self:GetAngles(), self:GetPos(),nil)
