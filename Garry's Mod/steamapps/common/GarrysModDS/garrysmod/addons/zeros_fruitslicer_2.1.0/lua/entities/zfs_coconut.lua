...
function ENT:Finish_VFX_SFX()
    ...
    zfs.f.CreateEffectTable(nil,  "zfs_sfx_coconut_finish", self, self:GetAngles(), self:GetPos(),nil)
    return self.BaseClass.Finish_VFX_SFX(self)
end

function ENT:Interact_VFX_SFX()

    zfs.f.CreateEffectTable("zfs_coconut",  "zfs_sfx_coconut", self, self:GetAngles(), self:GetPos(),nil)
