					...
					timer.Simple( CH_SupplyCrate.Config.MoneyBag_StealTime, function()
						...
						if bagtrace:GetClass() == "police_money_bag" then
							if caller:GetPos():DistToSqr( self:GetPos() ) < 10000 then
								caller:addMoney( self.BagMoney )
								DarkRP.notify( caller, 1, 5, DarkRP.formatMoney( self.BagMoney ) .." "..CH_SupplyCrate.Config.Lang["has been taken from the money stack!"][CH_SupplyCrate.Config.Language] )
