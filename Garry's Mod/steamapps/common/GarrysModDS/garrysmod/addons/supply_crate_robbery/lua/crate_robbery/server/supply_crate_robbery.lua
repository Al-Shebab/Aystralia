...
-- CH_SupplyCrate.Config.Lang["Your bitmining shelf is overheating!"][CH_SupplyCrate.Config.Language]
...
function CRATE_StartRobbery( ply, crate )
	...
	DarkRP.notify( ply, 1, 5, CH_SupplyCrate.Config.Lang["You have began a robbery on the police supply crate!"][CH_SupplyCrate.Config.Language] )
	...
	DarkRP.notify( ply, 1, 13, CH_SupplyCrate.Config.Lang["If you go to far away from the police supply crate, the robbery will also fail!"][CH_SupplyCrate.Config.Language] )
	...
	net.Start( "CRATE_RestartTimer" )
		net.WriteDouble( CH_SupplyCrate.Config.RobberyAliveTime * 60 )
	...
	timer.Simple( 10, function() -- FINALLY WE RESET STUFF
		if IsValid( crate ) then
			...
			net.Start( "CRATE_RestartCooldown" )
				net.WriteDouble( CH_SupplyCrate.Config.RobberyCooldownTime * 60 )
			net.Broadcast()
...
-- Robbery failure checks
local function CRATE_PlayerDeath( ply, inflictor, attacker )
	if ply.IsRobbingCrate then
	...
	for k, v in pairs( player.GetAll() ) do
		if v.IsRobbingCrate and IsValid( thecrate ) then
			if v:GetPos():DistToSqr( thecrate:GetPos() ) > tonumber( CH_SupplyCrate.Config.RobberyDistance ) then
				DarkRP.notify( v, 1, 5, CH_SupplyCrate.Config.Lang["You have moved to far away from the supply crate, and the robbery has failed!"][CH_SupplyCrate.Config.Language] )
...
local function CRATE_RobberyDisconnect( ply )
	if ply.IsRobbingCrate then			
		CRATE_RobberyFinished( false )
...
function CRATE_SpawnMoneyBag( ply )
	...
	CRATE_MoneyBag.BagMoney = CH_SupplyCrate.Content.Money
	CRATE_MoneyBag:Spawn()
	
	timer.Simple( 0.5, function()
		net.Start( "CRATE_UpdateMoneyBagAmt" )
			net.WriteEntity( CRATE_MoneyBag )
			net.WriteDouble( CRATE_MoneyBag.BagMoney )
		net.Broadcast()
	end )
end
...
function CRATE_SpawnShipments( ply )
	...
	local phys = CRATE_SpawnedShipment:GetPhysicsObject()
	phys:Wake()
