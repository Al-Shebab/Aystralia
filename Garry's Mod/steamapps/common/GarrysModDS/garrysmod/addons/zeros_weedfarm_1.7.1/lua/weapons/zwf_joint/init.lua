...
function SWEP:StopBongSound()
	...
	zwf.f.CreateNetEffect("zwf_joint_stop",self.Owner)
end
...
function SWEP:DoPrimaryAnims()
	...
	zwf.f.Timer_Create(timerID,1,1,function()
		...
		if IsValid(self) then
			...
			zwf.f.CreateNetEffect("zwf_joint_start",self.Owner)
...
function SWEP:SmokeFinish()
	if self.SmokeCount > 0 then
		...
		zwf.f.SmokeEffect(self:GetWeedID(),self.SmokeCount,self.Owner,25)
		self.SmokeCount = 0
	end
...
function SWEP:PlayDrawAnim()
	...
	zwf.f.Timer_Create(timerID,0.3,1,function()
		...
		if IsValid(self) and IsValid(self.Owner) and IsValid(self.Owner:GetActiveWeapon()) and string.sub( self.Owner:GetActiveWeapon():GetClass(), 1,8 )  then
...
function SWEP:RemoveJoint()
	...
	self.Owner:StripWeapon("zwf_joint")
end
