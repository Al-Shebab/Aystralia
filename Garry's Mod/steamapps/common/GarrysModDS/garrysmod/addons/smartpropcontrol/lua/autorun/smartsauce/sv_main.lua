...
main.bPropPhysics 		= true;
main.bFadingDoor		= true;
...
function main:GenerateVariableTable()
	...
	return {
		...
		bPropPhysics = self.bPropPhysics,
		bFadingDoor = self.bFadingDoor,
	};
end
...
/*
	Returns a table of all staff 
*/
function main:GetStaff()
...
function main:BlacklistEntity(ply, ent)
	...
	notify:NotifyPlayer(self:GetStaff(), string.format(smartsauce_config.lang.blacklistNote, ent:GetClass()), 4);
	self:Save();
end 
...
function main:UnblacklistEntity(ply, ent)
	...
	notify:NotifyPlayer(self:GetStaff(), string.format(smartsauce_config.lang.unblacklistNote, ent:GetClass()), 4);
end 
		...
		if (main.fadingDoorTools[tool]) then // this is the fading door tool!
			if (IsValid(tr.Entity)) then 
				...
				if (main:IsFadingDoorExploit(tr.Entity)) then 
					...
					notify:NotifyPlayer(main:GetStaff(), string.format(smartsauce_config.lang.fadingDoorCrash, ply:Nick()), 10);
					return false;
				end
			end
		end 
...
function main.OnPhysgunDrop(ply, ent)
	...
	if (main.bGhostProps) then 
		...
		timer.Simple(0, function() // Was 0.1s
			if (IsValid(ent)) then 
	...
	if (!main.bPropPhysics) then 
		timer.Simple(0, function()
			if (ent == NULL) then return end
			if (!IsValid(ent)) then return end
			if (ent:GetClass() != "prop_physics") then return end
			if (smartsauce_config.forcedPhysics[ent:GetModel()]) then return end
...
function main.EntityTakeDamage(ent, cDamageInfo)
	...
	if (IsValid(cDamageInfo:GetAttacker())) then 
		...
		*/
		if (!cDamageInfo:IsFallDamage()) then return true end
	end
end 
...
function main.ReceiveEdit(len, ply)
	...
	elseif(actionData.action == "main_save") then 
		...
		main.bPropPhysics = actionData.data.bPropPhysics;
		main.bFadingDoor = actionData.data.bFadingDoor;
		notify:NotifyPlayer(ply, smartsauce_config.lang.savedChanges, 4);
...
*/
function main.Crash()
	notify:NotifyPlayer(main:GetStaff(), smartsauce_config.lang.crashPrevented, 5);
end 
...
function main.StackerCrash(minge)
	...
	notify:NotifyPlayer(main:GetStaff(), string.format(smartsauce_config.lang.unfreezeCrash, minge:Nick()), 10);
end
...
function main.Init()
	...
	if (!main.bPropPhysics) then 
		...
		for k,v in next, ents.FindByClass("prop_physics") do 
			...
			if (smartsauce_config.forcedPhysics[v:GetModel()]) then continue end
