...
function menu:Open()
	...
	local propPhysics = smartDerma:DropDownSheet(smartsauce_config.lang.propPhysics, {"Enabled", "Disabled"}, self.data.bPropPhysics && 1 || 2, sheet:GetWide(), 20, function(s, index, value, data)
		if (value == "Enabled") then 
			self.data.bPropPhysics = true;
		else 
			self.data.bPropPhysics = false;
		end
	end);
	mList:AddItem(propPhysics);

	local fadingDoor = smartDerma:DropDownSheet(smartsauce_config.lang.antiFadingDoor, {"Enabled", "Disabled"}, self.data.bFadingDoor && 1 || 2, sheet:GetWide(), 20, function(s, index, value, data)
		if (value == "Enabled") then 
			self.data.bFadingDoor = true;
		else 
			self.data.bFadingDoor = false;
		end
	end);
	mList:AddItem(fadingDoor);
	...
	mList:AddItem(smartDerma:Button(smartsauce_config.lang.save, function()
		self:Edit("main_save", {
			...
			bPropPhysics = self.data.bPropPhysics,
			bFadingDoor = self.data.bFadingDoor,
		});
		frame.Dismiss();
	end));
