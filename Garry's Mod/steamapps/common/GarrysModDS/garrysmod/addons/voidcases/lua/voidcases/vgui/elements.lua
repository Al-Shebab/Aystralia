...
function VoidCases.FormatMoney(money, currency)
	...
	if (!currency) then
		currency = VoidCases.Config.Currency or table.GetKeys(VoidCases.Config.Currencies)[1]
	end

	local priceFont = (#tostring(money) > 6 and "VoidUI.R20") or "VoidUI.R24"
	local priceText = (tonumber(money) > 0 and (currency == "DarkRP" and DarkRP.formatMoney(tonumber(money))) or "$ " .. tonumber(money)) or "FREE"

	if (#tostring(money) > 7) then
		priceText = (currency == "DarkRP" and DarkRP.formatMoney(math.Round(money / 1000000, 2)) or "$ " .. math.Round(money / 1000000, 2)) .. " Mil"
	end

	local shortPoints = "Pts"

	if (currency == "Pointshop 1" or currency == "Pointshop 2 (Standard)" or currency == "Pointshop 2 (Premium)" or currency == "xStore" or currency == "Credits" or currency == "mTokens") then
		priceText = (tonumber(money) > 0 and tonumber(money) .. " " .. shortPoints) or "FREE"
		priceFont = "VoidUI.R22"

		if (currency == "mTokens") then
			shortPoints = "Tok."
		end

		if (#tostring(money) > 5) then
			priceText = math.Round(money / 1000, 2) .. "K Pts"
			priceFont = "VoidUI.R20"
		end

		if (#tostring(money) > 6) then
			priceText = math.Round(money / 1000000, 2) .. "M Pts"
			priceFont = "VoidUI.R20"
		end
	end
...
-- 	self.close:SetSize(math.ceil(ScrH() * 0.037), math.ceil(ScrH() * 0.037))
...
-- 	local navbarHeight = math.ceil(ScrH() * 0.0648)

-- 	local x, y = self:LocalToScreen(0,0)

-- 	BSHADOWS.BeginShadow()
-- 		surface.SetDrawColor(VoidUI.Colors.Primary)
-- 		surface.DrawRect(x,y,w,navbarHeight)
-- 	BSHADOWS.EndShadow(1, 1, 1, 150, 0, 0)

-- 	draw.SimpleText(self.title, "VoidUI.R48", w/2, navbarHeight/2 - 4, VoidUI.Colors.Gray, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

-- 	local drawColor = (self.close:IsHovered() and VoidUI.Colors.Red) or VoidUI.Colors.White

-- 	surface.SetDrawColor(drawColor)
-- 	surface.SetMaterial(VoidCases.Icons.Close)
-- 	surface.DrawTexturedRect(self.close.x,self.close.y,math.ceil(ScrH() * 0.037),math.ceil(ScrH() * 0.037))
-- end

-- vgui.Register("VoidCases.Frame", PANEL, "DFrame")
...
-- 	self:Dock(LEFT)
-- 	self:DockMargin(0, 0, 0, 0)
-- 	self:SetWide(math.Round(ScrW() * 0.052))
...
-- function PANEL:AddTab(text, icon, panel, bottomAlign)
-- 	local tab = self:Add("DButton")
-- 	tab:Dock((bottomAlign and BOTTOM) or TOP)
-- 	tab:SetTall(math.Round(ScrW() * 0.052))
-- 	tab:SetText("")
...
-- 	local iconSize = {35,35}

-- 	for k, v in pairs(table.GetKeys(VoidCases.IconSizes)) do
-- 		if (VoidCases.Icons[v] and VoidCases.Icons[v] == icon) then
-- 			iconSize = VoidCases.IconSizes[v]
-- 		end
-- 	end
...
-- 			col = VoidUI.Colors.Background
-- 			textCol = VoidUI.Colors.White
-- 			iconCol = VoidUI.Colors.White
-- 		end

-- 		local icoX = iconSize[1]
-- 		local icoY = iconSize[2]
...
-- 			surface.DrawTexturedRect(w/2-icoX/2,23,icoX,icoY)
-- 		end

-- 		draw.SimpleText(text, "VoidCases.S18", w/2, icoY+33, textCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
-- 	end
...
-- vgui.Register("VoidCases.Sidebar", PANEL, "DPanel")
...
function PANEL:SetItem(item, typ, smIcon)
	...
	timer.Simple(0, function ()
		...
		local currency = VoidCases.Config.Currencies[item.info.currency]
		if (currency) then
...
function PANEL:CalculateChance(editing)
	...
	// 76561198166995705 / 10^8 * sumOfAllChances
	local chanceSum = 0
