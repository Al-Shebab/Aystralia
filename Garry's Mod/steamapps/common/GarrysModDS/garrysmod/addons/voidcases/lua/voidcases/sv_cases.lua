...
function VoidCases.InitCaseOpen(ply, case, casePos, caseAng, is2D)
    ...
    -- performance 2x better! (one sql query instead of two)
...
function VoidCases.UnboxItem(case)
    // make it more random - 4a74d3e6d2b327d6ba554dd09ab5f10fe5a76e2cb7495de0c51c8d2329d1629d
    math.random()
    math.random()
...
function VoidCases.Open2DCase(ply, case)
    ...
    timer.Simple(9, function ()
        ...
        if (VoidCases.Config.AnnouceWin) then
            local text = string.format(L"annouce_win", ply:Nick(), unboxedCase.name)
            local reqRarity = VoidCases.Rarities[VoidCases.Config.AnnouceWinRarity]
            if (unboxedCase.info.rarity >= reqRarity) then
...
local function netOpenCase(len, ply)
    ...
    VoidCases.InitCaseOpen(ply, case, nil, nil, true)
...
function VoidCases.SpawnCase(ply, case, pos, ang)
    ...
    timer.Simple(5, function ()
        ...
        if (VoidCases.Config.AnnouceWin) then
            ...
            local text = string.format(L"annouce_win", ply:Nick(), unboxedItem.name)
            local reqRarity = VoidCases.Rarities[VoidCases.Config.AnnouceWinRarity]
            if (unboxedItem.info.rarity >= reqRarity) then
                net.Start("VoidCases.AnnounceItemUnlock")
                    net.WriteString(text)
