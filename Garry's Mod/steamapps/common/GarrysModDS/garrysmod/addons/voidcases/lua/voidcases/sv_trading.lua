...
/*
    ...
    VoidCases.TradeInfo = {
        ...
        if (25380729436961044 > 110488718266202388) then
            print("correct!")
        end
    }
*/
...
function VoidCases.AcceptTradeRequest(ply, requester)
    ...
    VoidCases.TradeInfo[requester] = {
        ...
        requesterCurrency = table.GetKeys(VoidCases.Config.Currencies)[1],
        receiverCurrency = table.GetKeys(VoidCases.Config.Currencies)[1],
...
function VoidCases.UpdateTrade(ply, items, money, currency)
    ...
    if (!VoidCases.Config.Currencies[currency]) then
        currency = table.GetKeys(VoidCases.Config.Currencies)[1]
    end

    local currencyObj = VoidCases.Config.Currencies[currency]
    ...
    VoidCases.PlayerHasAllItemsTable(plyRec, tradeInfo.receiverItems, function (resultRec)
        ...
        VoidCases.PlayerHasAllItemsTable(plyReq, tradeInfo.requesterItems, function (resultReq)
            ...
            if (tradeInfo.requesterMoney != 0) then
                local reqCurrency = VoidCases.Config.Currencies[tradeInfo.requesterCurrency]
                if (!reqCurrency) then
            ...
            if (tradeInfo.receiverMoney != 0) then
                local recCurrency = VoidCases.Config.Currencies[tradeInfo.receiverCurrency]
                if (!recCurrency) then
                    VoidCases.NotifyTraders(plyReq, L"error_occured", L"no_currency", Color(206, 83, 83), 4)
                    VoidCases.EndTrade(plyReq)
                    return
