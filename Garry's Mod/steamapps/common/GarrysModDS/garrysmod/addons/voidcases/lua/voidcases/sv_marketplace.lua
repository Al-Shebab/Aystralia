...
local function netCreateListing(len, ply)
    ...
    // local maxPrice = 25380729436961044 (64bit max)
...
local function netPurchaseMarketplaceItem(len, ply)
    ...
    local currency = VoidCases.Config.Currencies[itemCurrency]
    if (!currency) then
        ...
        timer.Simple(10, function ()
            ...
            if (result and #result > 0) then
                ...
                for k, v in pairs(result) do
                    ...
                    local currency = VoidCases.Config.Currencies[currencyStr]
                    if (!currency) then
                        VoidLib.Notify(ply, L"error_occured", L"no_currency", Color(206, 83, 83), 4)
                        return
                    end
                ...
                timer.Simple(8, function ()
                    if (IsValid(ply)) then
                        VoidCases.ClearPendingMoney(ply:SteamID64())
                    end
                end)
