...
function VoidCases.AutomaticItemsFeature()
    ...
    VoidCases.GetMostPurchasedItems(5, function (result)
        if (result and #result > 0) then
            ...
            VoidCases.BroadcastConfigData(VoidCases.Config)
        end
    end)
...
function VoidCases.InsertPurchase(sid64, item)
...
local function netPurchaseItem(len, ply)
    ...
    // Has money?
    local currency = VoidCases.Config.Currencies[item.info.currency or VoidCases.Config.Currency or table.GetKeys(VoidCases.Config.Currencies)[1] or "DarkRP"]
    if (!currency) then
    ...
    VoidCases.IsOnCooldown(ply, itemID, amount, function (result, time)
        ...
        VoidCases.InsertPurchase(ply:SteamID64(), itemID)
    end)
...
function VoidCases.EquipItem(ply, itemID, dontNotify)
    local item = VoidCases.Config.Items[itemID]
    if (!item or item.type != VoidCases.ItemTypes.Unboxable) then return end

    local action = VoidCases.Config.Actions[item.info.actionType]
    ...
    if (result and result > 0) then
        ...
        // Take item
        VoidCases.AddItem(sid64, itemID, -1, function ()
            ply.vc_lockedEquip = false
        end)

        // Network player inventory
        VoidCases.NetworkItem(ply, itemID, -1)
...
function VoidCases.IsOnCooldown(ply, itemID, _amount, cb)
    ...
    local query = VoidLib.SQL:Select("voidcases_purchases")
        ...
        query:Callback(function (result)
            local amount = result and #result or 0
...
local function netEquipItem(len, ply)
    ...
    local itemID = net.ReadUInt(32)
    VoidCases.EquipItem(ply, itemID)
