...
function PANEL:Init()
    ...
    self.itemPanel = itemContainer:Add("VoidUI.ScrollPanel")
    ...
    local itemWrapper = self.itemPanel:Add("VoidUI.Grid")
    ...
    for id, amount in pairs(self.invTbl or VoidCases.Inventory) do
        ...
        itemPanelB.DoClick = function ()
            ...
            if (item.type == VoidCases.ItemTypes.Unboxable) then
                ...
                if (item.info.actionType == "entity") then
                    self:GetParent():Remove()
                end
            end
        end
    ...
    itemWrapper:AutoSize()
    timer.Simple(0, function ()
        if (IsValid(itemWrapper)) then
            itemWrapper:AutoSize()
        end
    end)
