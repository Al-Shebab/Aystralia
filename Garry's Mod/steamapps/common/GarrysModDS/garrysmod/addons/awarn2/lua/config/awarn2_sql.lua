...
MySQLite_AWarn.initialize( AWarn.MySQLConfig.mysql_config )

local PlayerMeta = FindMetaTable("Player")

function PlayerMeta:GetTotalWarnings()
	return self.totalwarnings
end

function PlayerMeta:GetActiveWarnings()
...
function awarn_addwarning( uid, reason, admin )
	...
	date = os.date()
	local query = "INSERT INTO awarn_warnings VALUES (NULL, '" ..uid.. "', " ..admin.. ", " ..reason.. ", '" ..date.. "', '" .. AWarn.MySQLConfig.ServerKey .. "')"
	MySQLite_AWarn.query( query, function() end, function( error, q ) ServerLog( error ) end )
end

function awarn_incwarnings( ply )
	local uid = ply:SteamID64()
	ply.activewarnings = ply.activewarnings or 0
	...
	function( result, last )
		...
		ply.totalwarnings = wnum
		ply.activewarnings = ply.activewarnings + 1
	end,
	function( error, q )
		ServerLog( error )
	end )
...
function awarn_checkkickban( ply )
	...
	function( result, last )
		...
		if AWarn.Config.PunishmentSequence[wnum] then
			if AWarn.Config.PunishmentSequence[wnum].PunishmentType == "kick" then
				if kickon then
					...
					timer.Simple(1, function() awarn_kick( ply,AWarn.Config.PunishmentSequence[wnum].PunishmentMessage ) end )
					return
				end
			end
			
			if AWarn.Config.PunishmentSequence[wnum].PunishmentType == "ban" then
				if banon then
					ServerLog("AWarn: BANNING " .. ply:GetName() .. " FOR " .. AWarn.Config.PunishmentSequence[wnum].PunishmentLength .. " minutes!\n")
					...
					timer.Simple(1, function() awarn_ban( ply, AWarn.Config.PunishmentSequence[wnum].PunishmentLength, AWarn.Config.PunishmentSequence[wnum].PunishmentMessage ) end )
					return
				end			
			end
		end		
		...
		function( result, last )
			...
			if not result then
				...
				if tonumber(result[1].warnings) > 0 then
					...
					local query = "UPDATE awarn_playerdata SET warnings='" .. wnum .. "', lastwarn='" .. os.time() .."' WHERE unique_id='" ..uid.. "'"
					MySQLite_AWarn.query( query, function() end, function( error, q ) ServerLog( error ) end )
					if ply then AWSendMessage( admin, AWarn.localizations[loc].sql4 .. ply:GetName() ) end
				else
					if admin then AWSendMessage(admin, "AWarn: " .. AWarn.localizations[loc].sql5 ) end
				end
			end
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			if not result then
				...
				if IsValid(admin) then admin_name = admin:GetName() end
				
				ServerLog( "[AWarn] " .. admin_name .. " cleared all warnings from player: " .. ply:GetName() .. "\n" )
				if GetConVar("awarn_logging"):GetBool() then
					awarn_log( admin_name .. " cleared all warnings from player: " .. ply:GetName() )
				end
				
				ply.totalwarnings = 0
				ply.activewarnings = 0
			end
		end,
		function( error, q )
			ServerLog( error )
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			...
			if not result then
				...
				ServerLog( "[AWarn] " .. admin_name .. " cleared all warnings from player: " .. tostring(uid) .. "\n" )
				if GetConVar("awarn_logging"):GetBool() then
					awarn_log( admin_name .. " cleared all warnings from player: " .. tostring(uid) )
				end
			end
		end,
		function( error, q )
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			...
			if not result then
				...
				awarn_TryDecrementWarningFromID( tostring(result[1].unique_id) )
			end
...
function awarn_TryDecrementWarningFromID( pid )
	for k, v in pairs( player.GetAll() ) do
		if v:SteamID64() == pid then
			v.totalwarnings = v.totalwarnings - 1
			break
		end
	end
end
...
function awarn_sendofflineplayers( ply, all )
	local query = "SELECT * FROM awarn_offlinedata where warnings > 0 ORDER BY lastplayed DESC LIMIT 500"

	MySQLite_AWarn.query( query, 
		function( result, last )
			if not result then
				result = {}
				awarn_breakdowntableandsendoffline( result, ply )
			else
				awarn_breakdowntableandsendoffline( result, ply )
			end
		end,
...
function awarn_breakdowntableandsendoffline( tbl, ply )
	if not IsValid( ply ) then return end
	local newTable = {}
	local tblCount = 0
	local totalTables = 1
	...
	if #tbl == 0 then
		...
		while #tbl > 0 do
			if not newTable[totalTables] then newTable[totalTables] = {} end
			table.insert( newTable[totalTables], tbl[1] )
			table.remove( tbl, 1 )
			tblCount = tblCount + 1
			if tblCount >= 100 then
				tblCount = 0
				if #tbl > 0 then
		...
		for i = 1, totalTables do
			timer.Simple( 0.2 * i, function()
				if not IsValid( ply ) then return end
				net.Start("SendOfflinePlayers")
					net.WriteTable( newTable[i] )
				net.Send( ply )
			end )
		end
	end
...
function awarn_welcomebackannounce( ply )
	...
	local uid = ply:SteamID64() or "SINGLE_PLAYER"
	local query = "SELECT warnings FROM awarn_playerdata WHERE unique_id='" .. uid .. "'"
	ply.activewarnings = 0
	ply.totalwarnings = 0
	MySQLite_AWarn.query( query, 
...
function awarn_playerDisconnect( ply )
	if not IsValid( ply ) then return end
	local uid = ply:SteamID64()
	local plyNick = sql.SQLStr( ply:GetName() )
	local warnings = ply.totalwarnings or 0
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			...
			if not result then
				...
				local query = "UPDATE awarn_offlinedata SET playername=" .. plyNick .. ", warnings=" .. warnings .. ", lastplayed='" .. os.time() .. "' WHERE unique_id='" ..uid.. "'"
				MySQLite_AWarn.query( query, function() end, function( error, q ) ServerLog( error ) end )
			end
		end,
		function( error, q )
