...
include( 'includes/awarn2_menu.lua' )
include( 'includes/awarn2_skin.lua' )
...
function awarn_menu()
	...
	for _, v in pairs( player.GetAll() ) do
		AWarn.MenuFrame.PlayerList:AddLine( v:GetName() )
	end
...
function awarn_optionsmenu()
	...
	punishment_text2:SetText("addons/awarn2/lua/config/awarn_settings.lua")
...
net.Receive("SendPlayerWarns", function(length )
	...
	if ValidPanel( AWarn.MenuFrame ) then
		if AWarn.playerinfo then
			...
			for k, v in pairs(AWarn.playerinfo) do
				...
				AWarn.MenuFrame.WarningsList:AddLine(v.pid, v.admin, v.reason, v.server, FormatDateString( v.date ))
			end
	...
	if ValidPanel( AWarn.NewMenu.new_menu ) then
		if AWarn.playerinfo then
			...
			for k, v in pairs(AWarn.playerinfo) do
				if v.server == "NULL" then v.server = "UNKNOWN" end
				AWarn.NewMenu.new_menu.WarningsList:AddLine(v.pid, v.admin, v.reason, v.server, FormatDateString( v.date ) )
			end
...
function FormatDateString( str )
	...
	local array = string.Explode( " ", str )
	local arrayLen = #array
	local newStr = array[arrayLen] .. "-" .. array[arrayLen-3] .. "-" .. array[arrayLen-2] .. " " .. array[arrayLen-1]
	return newStr
end
...
function awarn_con_remwarn( ply, _, args )
	...
	awarn_removewarn( args[1] )
end
concommand.Add( "awarn_removewarn", awarn_con_remwarn )
