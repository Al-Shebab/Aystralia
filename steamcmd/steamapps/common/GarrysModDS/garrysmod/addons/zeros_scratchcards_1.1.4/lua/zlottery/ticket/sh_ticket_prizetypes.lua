...
AddPrizeType({
    ...
    func = function(ply, data)
        if SERVER and data.ezskin.ezs_skinid and data.ezskin.ezs_weaponclass then
            ...
            if SH_EASYSKINS.HasPurchasedSkin(ply,data.ezskin.ezs_skinid, data.ezskin.ezs_weaponclass) == true then
                local skin = SH_EASYSKINS.GetSkin(data.ezskin.ezs_skinid)

                local text = zlt.language["EasySkin_Owned"]
                text = string.Replace(text,"$SkinName",tostring(skin.dispName))
                text = string.Replace(text,"$WeaponClass",tostring(data.ezskin.ezs_weaponclass))
                zclib.Notify(ply, text, 3)

                return
            end
            SV_EASYSKINS.GiveSkinToPlayer(ply:SteamID64(), data.ezskin.ezs_skinid, {data.ezskin.ezs_weaponclass})
        end
    end
})
