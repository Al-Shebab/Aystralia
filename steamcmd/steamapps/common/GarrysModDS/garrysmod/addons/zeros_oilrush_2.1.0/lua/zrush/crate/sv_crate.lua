...
function zrush.Machinecrate.OpenShop(Machinecrate, ply)
	if (not zclib.Player.IsOwner(ply, Machinecrate)) then
...
function zrush.Machinecrate.Buy(ent, machineshopId, ply)
	if not zclib.Player.IsOwner(ply, ent) then return end
...
net.Receive("zrush_Machinecrate_Build", function(len, ply)
	...
	if (IsValid(ent) and zclib.Player.IsOwner(ply, ent) and ent:GetClass() == "zrush_machinecrate" and zclib.util.InDistance(ent:GetPos(), ply:GetPos(), 2000)) then
