...
function zrmine.f.Player_CleanUpEnts(steamID)
    for k, v in pairs(zrmine.EntList) do
        if IsValid(v) and table.HasValue(zrms_DeleteEnts, v:GetClass()) and zrmine.f.GetOwnerID(v) == steamID then
            v:Remove()
        end
    end
end
...
hook.Add("player_disconnect", "a_zrmine_player_disconnect", function(data)
    local steamid
    ...
    zrmine.f.Player_Remove(steamid)

    zrmine.f.Player_CleanUpEnts(steamID)
end)

hook.Add("PlayerChangedTeam", "a_zrmine_PlayerChangedTeam", function(ply, before, after)
    if IsValid(ply) then
        if zrmine.config.Jobs[before] then
            ply:zrms_ResetMetalBars()
        end

        zrmine.f.Player_CleanUpEnts(zrmine.f.Player_GetID(ply))
    end
end)
