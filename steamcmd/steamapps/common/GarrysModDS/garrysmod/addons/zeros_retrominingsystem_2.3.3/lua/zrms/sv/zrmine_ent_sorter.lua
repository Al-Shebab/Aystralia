...
function zrmine.f.Sorter_SendResources(Sorter)
	...
	local sendFilter = zrmine.f.Calculate_AmountCap(Sorter.StoredResource[Sorter.FilterType], Sorter.WorkAmount)

	// Do we have a module for the Filter load?
	local cm01 = Sorter:GetModuleChild01()
	if (IsValid(cm01) and sendFilter > 0) then
		Sorter.StoredResource[Sorter.FilterType] = Sorter.StoredResource[Sorter.FilterType] - sendFilter

		sendTable[Sorter.FilterType] = sendFilter

		zrmine.f.General_ReceiveResources(cm01,sendTable)
		zrmine.f.CreateInsertEffect(cm01, Sorter.FilterType)
	end
...
function zrmine.f.Sorter_FindValidResource(Sorter)
	...
	for k, v in pairs(Sorter.StoredResource) do
		...
		if (sendAmount > 0 and k ~= Sorter.FilterType) then
			validResource = k
			break
		end
	end
