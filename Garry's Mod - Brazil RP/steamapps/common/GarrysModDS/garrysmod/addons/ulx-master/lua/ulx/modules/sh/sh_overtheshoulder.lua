...
local function ThirdPerson(ply, pos, ang)
    ...
    local targetpos = pos + ang:Forward() * -36 + ang:Right() * 24 + ang:Up() * 4
...
local function ThirdPersonOn(ply)
    net.Start("ots_on")
...
local function ThirdPersonOff(ply)
	net.Start("ots_off")
    net.Send(ply)
...
cmd:defaultAccess(ULib.ACCESS_USER) --Must ACCESS_SUPERADMIN, ADMIN, or USER.
