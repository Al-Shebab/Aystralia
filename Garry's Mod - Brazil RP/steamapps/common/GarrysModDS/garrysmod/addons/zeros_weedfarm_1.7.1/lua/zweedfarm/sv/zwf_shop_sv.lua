...
function zwf.f.Shop_Item_MaxCheck(ply,itemdata)
    local count = 0
    for k, v in pairs(zwf.EntList) do
        if IsValid(v) and v:GetClass() == itemdata.class and zwf.f.IsOwner(ply, v) then
            count = count + 1
        end
    end
...
function zwf.f.BuyItem(ply, pCat, pID)
    ...
    if zwf.f.Shop_Item_MaxCheck(ply,itemData) then
